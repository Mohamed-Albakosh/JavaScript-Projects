...
function Input_Decimal(dot) {
    // this ensures that accidental clickng of the decimal point
...
function Handle_Operator (Next_Operator) {
    ...
    }else if (operator) { // chicks if an operator already exists
...
Update_Display();
// this section monitors buttom clicks
const keys = document.querySelector('.calculator-keys');
keys.addEventListener('click', (event) => {
    // the target variable is an object that represents the element
    // that was clicked
    const { target } = event;
    // if the element that was clicked on is not a buttom, exit the function
